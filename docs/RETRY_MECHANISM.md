...
- **STUCK PROCESSING**: Documents in PROCESSING state for >1 hour (configurable)

**Implementation**: `worker/tasks_refactored.py` lines 84-102
...
# Check for stuck PROCESSING documents (>1 hour old)
if not documents:
...
**Process**:
1. Validates run is in retryable state (FAILED, PARTIAL_SUCCESS, or stuck PROCESSING)
2. Resets FAILED documents to PENDING
3. Resets stuck PROCESSING documents to PENDING
4. Resets run status to QUEUED
5. Enqueues new processing job

**Implementation**: `api/main.py` lines 548-625
...
- **Stuck Document Threshold**: 1 hour (hardcoded, can be made configurable)
...
**Error Responses**:
- `400 Bad Request`: Run is still actively processing
